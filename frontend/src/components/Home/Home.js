...
const Home = () => {
    ...
    useEffect(() => {
        const createInstance = () => {
            if (window.ethereum) {
                ...
                let _instance = new _web3.eth.Contract(
                    ABI,
    ...
    const getFromSC = async (e) => {
        ...
        const price = await instance.methods
            .getStockPrice(web3.utils.fromAscii(searchSymbol))
            .call();
        const volume = await instance.methods
            .getStockVolume(web3.utils.fromAscii(searchSymbol))
            .call();
            ...
            <div className="container">
                <div>
                    <span className="label">
                        Enter stock symbol to add it to smart contract
                    </span>
                    <form className="form" onSubmit={addToSC}>
                        ...
                        <input
                            type="submit"
                            className="button"
                            value="Add to smart contract"
                        />
                    </form>
                </div>
                <div style={{ marginTop: "10px" }}>
                    <span className="label">
                        Enter stock symbol to get price and volume from smart
                        contract
                    </span>
